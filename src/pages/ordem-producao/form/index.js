...
import { addOrdemProducao, editOrdemProducao } from '../../../store/actionCreators/ordemProducaoAction';
import { filterPessoa } from '../../../store/actionCreators/pessoaAction';

class OrdemProducaoForm extends Component {
    ...
    async componentDidMount() {
        ...
        if (this.state.id) {
            const response = await api.get("/ordens-producao/" + this.props.match.params.id);
            let itens = [];
    ...
    render() {
        ...
        return (
            <div>
                <Header as="h1">
                    {text('ordem-producao.titulo-form')}
                </Header>
                            ...
                            >
                                ...
                                <Form.Group widths='equal'>
                                    ...
                                    <div className="field required">
                                        <label>{text("ordem-producao.numero")}</label>
                                        <InputText
                                            disabled={id}
                                            onChange={e => {
                                                handleChange(e)
                                    ...
                                    <div className="field required">
                                        <label>{text("ordem-producao.data")}</label>
                                        <InputMask
                                            mask="99/99/9999"
                                            slotChar="dd/mm/yyyy"
                                            onChange={e => {
                                    ...
                                    <div className="field required">
                                        <label>{text("ordem-producao.fornecedor")}</label>
                                        <Dropdown
                                            ...
                                            placeholder={text("ordem-producao.fornecedor-placeholder")}
                                ...
                                <Form.Group>
                                    <div className="field" style={{ width: '100%' }}>
                                        <label>{text("ordem-producao.descricao")}</label>
                                        <InputTextarea
                                            rows={4}
                                            onChange={e => {
                                                handleChange(e)
                                ...
                                <Segment attached>
                                    <Form.Group widths='equal'>
                                        ...
                                        <div className="field required">
                                            <label>{text("ordem-producao.modelo-dispositivo")}</label>
                                            <Dropdown
                                                ...
                                                placeholder={text("ordem-producao.modelo-dispositivo-placeholder")}
                                                noResultsMessage={text("geral.nenhum-resultado-encontrado")}
                                            />
                                        </div>
                                        <div className="field required">
                                            <label>{text("ordem-producao.quantidade")}</label>
                                            <InputText
                                                onChange={e => {
                                        ...
                                        <div className="field required">
                                            <label>{text("ordem-producao.valor-unitario")}</label>
                                    ...
                                    <Table celled striped selectable sortable >
                                        <Table.Header>
                                            <Table.Row>
                                                <Table.HeaderCell>{text("ordem-producao.modelo-dispositivo")}</Table.HeaderCell>
                                                <Table.HeaderCell>{text("ordem-producao.quantidade")}</Table.HeaderCell>
                                                <Table.HeaderCell>{text("ordem-producao.valor-unitario")}</Table.HeaderCell>
...
const mapStateToProps = state => {
    return {
        ...
        listaNotasFiscaisCompra: state.ordemProducaoReducer.listaNotasFiscaisCompra,
        sucesso: state.ordemProducaoReducer.sucesso,
        erro: state.ordemProducaoReducer.erro,
        executando: state.ordemProducaoReducer.executando,
        erroInsert: state.ordemProducaoReducer.erro,
    }
}

const mapDispatchToProps = dispatch => {
    return {
        addOrdemProducao: (ordemProducao) => dispatch(addOrdemProducao(ordemProducao)),
        editOrdemProducao: (ordemProducao, id) => dispatch(editOrdemProducao(ordemProducao, id)),
        filterPessoa: (type, filterValue) => dispatch(filterPessoa(type, filterValue)),
        setErro: (erro) => dispatch({ type: actions.ERRO_ORDEM_PRODUCAO, payload: erro })
    };
};

export default connect(mapStateToProps, mapDispatchToProps)(OrdemProducaoForm);