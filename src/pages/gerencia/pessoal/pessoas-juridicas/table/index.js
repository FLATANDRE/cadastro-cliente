...
class PessoasJuridicasTable extends Component {
    ...
    handleSearch = e => {
        ...
        this.props.filterPessoa(pessoaTypes.PESSOA_JURIDICA, busca);
        this.setState({ sucesses: null })
    }
        ...
        return (
            <div>
                <Grid stackable columns={2} >
                    <Grid.Column>
                        <Header as="h1">{text('pessoa.titulo-pj')}</Header>
                    </Grid.Column>
                        ...
                        <Form size='large' className="form">
                            <Form.Group widths='equal'>
                                ...
                                <Button
                                    ...
                                    size='large'
                                    as={Link}
                                    to="/gerencia/pessoal/pessoa-juridica/"
                                >
                ...
                <Table celled striped selectable sortable >
                    <Table.Header>
                        <Table.Row>
                            ...
                            >{text("pessoa.nome-fantasia")}</Table.HeaderCell>

                            <Table.HeaderCell
                                sorted={this.state.sort === 'cnpj' ? this.state.sortDirection : null}
                                onClick={this.handleSort('cnpj')}
                            >{text("pessoa.cnpj")}</Table.HeaderCell>
                        ...
                        {!this.props.consultando && this.props.listaPessoasFiltrada && Array.isArray(this.props.listaPessoasFiltrada) &&
                            this.props.listaPessoasFiltrada.map((item) =>
                                <Table.Row key={item.id}>
                                    ...
                                    <Table.Cell>{item.nomeFantasia ? item.nomeFantasia : text("geral.nao_informado")}</Table.Cell>
                                    <Table.Cell>{item.cnpjFormatado}</Table.Cell>
                                    <Table.Cell>
                                        <Button icon size='mini' as={Link}
                                            to={"/gerencia/pessoal/pessoa-juridica/" + (item.id)}
                                        > <Icon name='edit' />
                                        </Button>
                                    </Table.Cell>
                                </Table.Row>
...
const mapDispatchToProps = dispatch => {
    return {
        fetchAllPessoa: (busca = '', page = 0, sort = '', sortDirection = 'asc') => dispatch(fetchAllPessoa(pessoaTypes.PESSOA_JURIDICA, busca, page, sort, sortDirection)),
        filterPessoa: (type, busca) => dispatch(filterPessoa(type, busca)),
    };
};

export default connect(mapStateToProps, mapDispatchToProps)(memo(PessoasJuridicasTable));
